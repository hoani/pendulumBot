from pendulumBot.bot import ahrs
...
class TestAhrsDynamicUpdates:
  ...
  def setup_method(self):
    ...
    self.imu_data = imuData.ImuData(
        vect.Vec3(0.0, 0.0, ahrs.AhrsConstants.DEFAULT_GRAVITY),
        vect.Vec3(0.0, 0.0, 0.0),
        vect.Vec3(40.0, 0.0, 0.0)
      )
...
class TestAhrsCalibrate:
  ...
  def test_calibration_magnetometer_at_45_pitch(self):
    ...
    self.imu_data.accelerometer.z = ahrs.AhrsConstants.DEFAULT_GRAVITY * np.sqrt(0.5)
  ...
  def test_calibration_acceleration(self):
    self.imu_data.accelerometer.x = 10.0 * np.sin(20.0 * ahrs.AhrsConstants.DEG_TO_RAD)
    self.imu_data.accelerometer.y =  0.0
    self.imu_data.accelerometer.z = 10.0 * np.cos(20.0 * ahrs.AhrsConstants.DEG_TO_RAD)
    self.ahrs.start_calibrate()
...
class TestAhrsStill:
  ...
  def test_accel_average(self):
    self.imu_data.accelerometer.x = -9.81 * np.sin(20.0 * ahrs.AhrsConstants.DEG_TO_RAD)
    self.imu_data.accelerometer.y = 0.0
    self.imu_data.accelerometer.z = 9.81 * np.cos(20.0 * ahrs.AhrsConstants.DEG_TO_RAD)
  ...
  def test_movement_resets_averaging(self):
    ...
    self.imu_data.gyroscope.x = ahrs.AhrsConstants.CALIBRATION_GYRO_MAX # Movement - should prevent us from averaging
