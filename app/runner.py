from dio.comms import btServer, tcpServer

import time
import app.control.modes as RobotControl
from core import motorPair, ahrs
from dio.comms import commandRegister, commandCallbacks, remoteLogger
...
class RobotRunner:
    def __init__(self, settings):
        ...
        self.codec = leap.Codec('app/config/protocol.json')
        self.cpu_usage = 0.0
        ...
        if (simulate):
            from dio.driver.dummy import motors, imu, adc, servo
            self.adc = adc
            self.servo = servo
            self.rcpy = None
            left = motors.DcMotor()
            right = motors.DcMotor()
            self.imu = imu.Imu()

        else:
            import rcpy
            from dio.driver.rcpy import motors, imu, adc, servo
            ...
            self.imu = imu.Imu(
                mapping=vec3.Vec3(2, 0, 1),
                sign=vec3.Vec3(-1, -1, 1)
            )
        ...
        try:
            ...
            if (simulate is False):
                self.sockets.append(btServer.btServer(
                    hostBtMACAddress, hostBtPort))
            self.sockets.append(tcpServer.TcpServer(
                hostTcpIpAddress, hostTcpPortCommand))

            self.logger_socket = tcpServer.TcpServer(
                hostTcpIpAddress, hostTcpPortLogging)
            self._remote_logger_setup()

            self.robo = RobotControl.RobotControl(pair)
            self.pitch_pid = pid.Pid(kp=1.0, ki=0.0, kd=0.0, setpoint=0.0)
            self.ahrs = ahrs.AhrsTwoWheeled()

            self.pendulum_control = pendulum.PendulumPid(
                self.pitch_pid, self.ahrs, limit=5.0)

            self.robo.add_controller(
                RobotControl.RobotControl.STATE_PENDULUM,
                self.pendulum_control)
            ...
            self.callback_objs = [
                commandCallbacks.PendulumCallbacks(
                    self.robo, self.pendulum_control),
    ...
    def run(self, update_period_ms):
        ...
        next_ms = (datetime.datetime.now().timestamp()
                   * 1000.0) + update_period_ms
        ...
        try:
            while self.check_exit_conditions():
                ...
                # Only used to determine if a connection has dropped
                _ = self.logger_socket.recv()
                ...
                for sock in self.sockets:
                    ...
                    if rx is not None:
                        ...
                        (last_data, packets) = \
                            self.codec.decode(last_data + data)

                        # To do - respond NAK to a group with unknown command
                ...
                if current_publish_ms - last_publish_ms >= publish_period_ms:
                    ...
                    self.battery_v = self.adc.Adc.battery_voltage()
    ...
    def _calculate_cpu_usage(self, last_s, update_period_s):
        ...
        print(('\r'
               ...
               ).format(delta_meas_s, self.cpu_usage, self.delta_max_s,
                        self.max_cpu_usage), end='')
    ...
    def _publish_subscribed_packets(self, imu_data, angles):
        ...
        ahrs_packet = leap.Packet(
            'pub', 'ahrs/angle', (angles.pitch, angles.yaw))
    ...
    def _remote_logger_update(self, imu_data, angles, delta_ms):
        self.remote_logger.set("timeMs", "{:d}".format(
            self.remote_logger.current_ms))
        self.remote_logger.set(
            "imuAccX", "{:0.03f}".format(imu_data.accelerometer.x))
        self.remote_logger.set(
            "imuAccY", "{:0.03f}".format(imu_data.accelerometer.y))
        self.remote_logger.set(
            "imuAccZ", "{:0.03f}".format(imu_data.accelerometer.z))
        self.remote_logger.set(
            "imuGyrX", "{:0.02f}".format(imu_data.gyroscope.x))
        self.remote_logger.set(
            "imuGyrY", "{:0.02f}".format(imu_data.gyroscope.x))
        self.remote_logger.set(
            "imuGyrZ", "{:0.02f}".format(imu_data.gyroscope.x))
        self.remote_logger.set(
            "imuMagX", "{:0.02f}".format(imu_data.magnetometer.x))
        self.remote_logger.set(
            "imuMagY", "{:0.02f}".format(imu_data.magnetometer.x))
        self.remote_logger.set(
            "imuMagZ", "{:0.02f}".format(imu_data.magnetometer.x))
        self.remote_logger.set("ahrsPitch", "{:0.02}".format(angles.pitch))
        self.remote_logger.set("ahrsYaw", "{:0.02}".format(angles.yaw))
        self.remote_logger.set("motorL", "{:0.02}".format(
            self.robo.get_wheels_duty()[0]))
        self.remote_logger.set("motorR", "{:0.02}".format(
            self.robo.get_wheels_duty()[1]))
