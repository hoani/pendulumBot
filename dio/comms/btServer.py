try:
    ...
    class btServer(bluetooth.BluetoothSocket):
        def __init__(self, mac_addr, port):
            print("Initializing bluetooth on ",
                  mac_addr, ":{:05}".format(port))
        ...
        def accept_connections(self):
            try:
                ...
                for i, (stored_client, stored_addr) in enumerate(self.clients):
                    if (stored_addr == addr):
                        print("Client reconnected ", addr)
                        self.clients[i] = (client, addr)
                        break
    ...
    if __name__ == "__main__":
        # The MAC address of a Bluetooth adapter on the server. The server
        # might have multiple Bluetooth adapters.
        hostMACAddress = '38:D2:69:E1:11:CB'
        port = 3
        backlog = 1
        size = 1024
